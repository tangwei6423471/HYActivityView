...
- (void)setup
{
    ...
    self.textLabel.textColor = COLOR_MAIN_GREEN;
...
//    self.buttonSpace = (self.referView.bounds.size.width - BUTTON_VIEW_SIDE * number) / (number + 1);
    // 三方登录，二次登录的时候 number3~5 怪异
    self.buttonSpace = (Screen_width - BUTTON_VIEW_SIDE * number) / (number + 1);
...
//    self.backgroundColor = [UIColor redColor];
    ...
    self.titleLabel.textAlignment = 1;// alvin 居中
    self.titleLabel.font = [UIFont systemFontOfSize:16.f];
    self.titleLabel.textColor = COLOR_MAIN_GREEN;
...
//    [self.cancelButton setTitle:@"取 消" forState:UIControlStateNormal];
    ...
    [self.cancelButton setTitleColor:[UIColor colorWithRed:0.918 green:0.204 blue:0.200 alpha:1.000] forState:UIControlStateNormal];
    ...
    self.lineView.backgroundColor = [UIColor colorWithRed:0.902 green:0.898 blue:0.863 alpha:1.000];
    ...
    self.lineView.translatesAutoresizingMaskIntoConstraints = NO; // 代码autoLayout需要设置这个，不然走XIB模式
...
- (void)updateConstraints
{
    ...
    NSLayoutConstraint *constraint = nil;
    // alvin 20150319
    NSDictionary *views = @{@"contentView": self.contentView, @"closeButton": self.closeButton, @"titleLabel": self.titleLabel, @"cancelButton": self.cancelButton, @"iconView": self.iconView, @"view": self, @"referView": self.referView,@"lineView":self.lineView};
    ...
    // alvin 20150319
    constraints = [NSLayoutConstraint constraintsWithVisualFormat:@"V:|-8-[titleLabel(==30)]-[iconView]-[lineView(==1)]-[cancelButton(==30)]-8-|" options:0 metrics:nil views:views];
    [self.contentView addConstraints:constraints];
...
- (void)prepareForShow
{
    if(self.workingNumberOfButtonPerLine == 0){self.workingNumberOfButtonPerLine = 1;}
    //计算行数
